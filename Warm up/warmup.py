...
def f_default_arg_mutable(x:int = None, data  : List[int] = None) -> List[int]: #param_name:type->type
    if data is None :
        return [x]
    elif len(data) < 3 and data is not None:
        data.append(x)
...
def f_str_formatting(array:List[float])->str:
    new_array : str=''
    for a in array[:-1]:
        new_array = new_array + '{:.2f} - '.format(a)
    new_array = new_array + '{:.2f}'.format(array[-1])
    return new_array
    pass
